...
target "rewriter" {
  dockerfile = "Dockerfile.rewriter"
  target = "without_tag"
  tags = [
    "${CI_REGISTRY_IMAGE}:${TAG}-rewriter"
  ]
  args = {
    BUILD_TIME = "${BUILD_TIME}",
    GIT_COMMIT_SHA = "${GIT_COMMIT_SHA}",
...
target "rewriter-ci" {
  inherits = ["rewriter"]
  cache-from = [
    "type=registry,ref=${registry-name("rewriter", "cache")}:latest"
  ]
  cache-to = [
    "type=registry,mode=max,ref=${registry-name("rewriter", "cache")}:latest"
  ]
  tags = [
    "${CI_REGISTRY_IMAGE}:${TAG}-rewriter",
    equal(CI_COMMIT_BRANCH, CI_DEFAULT_BRANCH) ? "${CI_REGISTRY_IMAGE}:latest-rewriter": "",
    notempty(CI_COMMIT_TAG) ? "${CI_REGISTRY_IMAGE}:${CI_COMMIT_TAG}-rewriter": ""
  ]
  output = [
    "type=registry"
  ]
