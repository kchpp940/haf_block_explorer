...
| `/metadata/endpoints` | GET | Get all API endpoints metadata |

#### Metadata Endpoints

The metadata endpoint provides a machine-readable list of all API endpoints, including their paths, parameters, return types, and version information. Each entry includes an `installed` flag that is `true` only when the backing SQL function exists in `pg_proc` **and** its parameter names and return type match the OpenAPI declaration. When `installed` is `false`, the `mismatch_reason` field explains why — possible values:

| mismatch_reason | Meaning |
|-----------------|---------|
| `function_not_found` | No function with that schema.name exists in `pg_proc` |
| `parameter_count_mismatch:openapi=N,db=M` | OpenAPI declares N parameters but the DB function has M |
| `parameter_name_mismatch:openapi=X,db=Y` | A parameter name differs between OpenAPI and `proargnames` |
| `return_type_mismatch:openapi=X,db=Y` | The declared return type doesn't match `prorettype` |
| `invalid_operation_id` | `operationId` is missing or not in `schema.function` format |

Multiple reasons are joined with `; `.

This is useful for frontend applications to automatically generate navigation menus and API client code, and for ops to detect installation drift.

**Get All Endpoints Metadata:**
```bash
curl "http://localhost:3000/hafbe-api/metadata/endpoints"
```
...
**Filter for installed endpoints only:**
```sql
SELECT (e).path, (e).rpc_function, (e).installed
FROM (
  SELECT unnest(endpoints) AS e
  FROM hafbe_endpoints.get_endpoints_metadata()
) sub
WHERE (e).installed = true;
```

**Find endpoints with mismatches:**
```sql
SELECT (e).path, (e).rpc_function, (e).mismatch_reason
FROM (
  SELECT unnest(endpoints) AS e
  FROM hafbe_endpoints.get_endpoints_metadata()
) sub
...
`/metadata/consistency` validates that three layers of endpoint deployment are in sync:

| Layer | Source | Check |
|-------|--------|-------|
| OpenAPI spec | `hafbe_endpoints.root()` | Declared paths and operationIds |
| Database | `pg_proc` + signature check | Function exists with matching args/return type |
| Rewrite rules | `hafbe_backend.rewrite_rules` table | Route is exposed and points to correct function |

**Possible issue types:**

| issue_type | Meaning |
|------------|---------|
| `openapi_not_installed` | In OpenAPI spec but DB function missing or signature mismatch |
| `installed_not_rewritten` | DB function exists but no rewrite rule exposing it |
| `rewritten_not_in_openapi` | Rewrite rule exists but path missing from OpenAPI spec |
| `rewrite_target_mismatch` | Rewrite points to different function than operationId |
| `rewrite_rules_not_loaded` | The mirror table is empty — run reload script |
| `rewrite_rules_stale` | Mirror is out of date — rules changed but not reloaded |

> **Self-protection for metadata endpoints**
>
> `/metadata/consistency` will report `rewrite_rules_stale` (not `installed_not_rewritten`)
> when it detects that `/metadata/endpoints` or itself are in OpenAPI but missing
> from the rewrite mirror. This avoids false positives when you've added the metadata
> endpoints to the code but haven't reloaded the rewrite rules yet.

**Run self-check:**
```bash
curl "http://localhost:3000/hafbe-api/metadata/consistency"
```
...
```json
{
  ...
  "issues": [
    {
      "issue_type": "openapi_not_installed",
      "path": "/experimental/feature",
      "rpc_function": "hafbe_endpoints.get_experimental_feature",
      "details": "Function does not exist in pg_proc"
    },
    {
      "issue_type": "installed_not_rewritten",
      "path": "/accounts/{account-name}/new-endpoint",
      "rpc_function": "hafbe_endpoints.get_new_endpoint",
      "details": "Endpoint is in OpenAPI spec but has no rewrite rule"
    }
  ]
}
```
...
#### Rewrite Rule Mirror Management

The consistency check reads rewrite rules from the `hafbe_backend.rewrite_rules`
mirror table, NOT directly from the nginx config. Whenever you edit
`endpoints/rewrite_rules.conf`, you must reload the mirror:

**Option 1 — Use the helper script:**
```bash
POSTGRES_ACCESS="postgresql:///haf_block_log" scripts/reload_rewrite_rules.sh
```

**Option 2 — SQL:**
```sql
SELECT hafbe_backend.load_rewrite_rules(
  pg_read_file('/path/to/endpoints/rewrite_rules.conf')
);
```

The mirror is automatically loaded during `install_app.sh`, and
`openapi_rewrite.sh` prints a reminder every time it regenerates the schema.

**How to diagnose stale mirror:**
```json
{
  "issues_found": 1,
  "issues": [
    {
      "issue_type": "rewrite_rules_stale",
      "path": "/metadata/consistency",
      "rpc_function": "hafbe_endpoints.get_endpoints_consistency",
      "details": "Metadata endpoint missing from rewrite rules mirror — run `SELECT hafbe_backend.load_rewrite_rules('...')` to reload after editing rewrite_rules.conf"
    }
  ]
}
```
