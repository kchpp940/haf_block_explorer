...
/*
 * get_account_proxies_power_endpoint: Endpoint helper for proxy power.
 *
 * Uses the account_query_context for account validation and pagination.
 *
 * PARAMETERS:
 *   _account_name - The proxy account name
 *   _page         - Page number (1-based)
 *   _sort         - Sort field: 'account', 'proxy_date', 'proxied_vests'
 *   _direction    - Sort direction: 'asc' or 'desc'
 *
 * RETURNS: SETOF proxy_power records
 *
 * NOTE: Cache control (5s TTL) and account validation are handled by context.
 */
CREATE OR REPLACE FUNCTION hafbe_backend.get_account_proxies_power_endpoint(
    _account_name TEXT,
    _page         INT,
    _sort         hafbe_backend.order_by_proxy,
...
DECLARE
    ...
    __flags hafbe_backend.account_validation_flags := (FALSE, FALSE, TRUE);
BEGIN
    __ctx := hafbe_backend.account_context_build_paginated(
        _account_name, _page, hafbe_backend.default_max_page_size(),
        hafbe_backend.default_max_page_size(), __flags, 5
    );
