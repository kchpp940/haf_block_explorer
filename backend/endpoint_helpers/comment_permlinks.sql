...
/*
 * get_comment_permlinks_endpoint: Retrieves comment permlinks with block range filtering.
 *
 * Consolidated helper for the get_comment_permlinks endpoint. Uses the
 * account_query_context for account validation, block range normalization,
 * and pagination handling.
 *
 * PARAMETERS:
 *   _account_name  - The author account name
 *   _comment_type  - Filter: 'post', 'comment', or 'all'
 *   _page          - Page number (1-based)
 *   _page_size     - Number of permlinks per page
 ...
 *
 * RETURNS: hafbe_backend.permlink_history with paginated permlinks
 *
 * NOTE: Cache control, block range handling, and validations are handled
 *       by the context builder.
 */
CREATE OR REPLACE FUNCTION hafbe_backend.get_comment_permlinks_endpoint(
    _account_name  TEXT,
    _comment_type  hafbe_backend.comment_type,
    _page          INT,
    _page_size     INT,
...
DECLARE
    ...
    __flags hafbe_backend.account_validation_flags := (TRUE, TRUE, TRUE);
BEGIN
    __ctx := hafbe_backend.account_context_build_block_range(
        _account_name, _from_block, _to_block,
        _page, _page_size, _max_page_size, __flags
    );
