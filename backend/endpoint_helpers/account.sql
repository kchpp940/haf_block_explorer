...
-- =============================================================================
-- SECTION 6: Endpoint Helper Function
-- =============================================================================

/*
 * get_account_endpoint: Retrieves complete account information.
 *
 * Consolidated helper for the get_account endpoint. Uses the account_query_context
 * for account validation, then assembles all account data from various
 * backend helpers into the hafbe_backend.account type.
 *
 * PARAMETERS:
 *   _account_name - The account name to retrieve
 *
 * RETURNS: hafbe_backend.account with all account fields populated
 *
 * NOTE: Cache control is handled by the context builder.
 */
CREATE OR REPLACE FUNCTION hafbe_backend.get_account_endpoint(
    _account_name TEXT
)
...
BEGIN
    ...
    RETURN (
        ...
        EXISTS (SELECT NULL FROM hafbe_app.current_witnesses WHERE witness_id = __ctx.account_id)
    )::hafbe_backend.account
    FROM
        btracker_endpoints.get_account_balances(__ctx.account_name)      _result_balance,
        reptracker_endpoints.get_account_reputation(__ctx.account_name)  _result_reputation,
        hafbe_backend.get_json_metadata(__ctx.account_id)              _result_json_metadata,
        hafbe_backend.get_account_parameters(__ctx.account_id)         _result_parameters,
        hafbe_backend.get_account_witness_votes(__ctx.account_id)      _result_votes,
        hafbe_backend.get_account_proxy(__ctx.account_id)              _result_proxy,
        hafbe_backend.get_account_ops_count(__ctx.account_id)          _result_count,
        hafbe_backend.get_account_proxied_vsf_votes(__ctx.account_id)  _result_proxied_votes;
END
$$;
